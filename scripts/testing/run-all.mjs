...
//   node scripts/testing/run-all.mjs --no-local-secrets   排除需要本机机密的测试（CI 用）
//
...
// 需要只存在于维护者本机的机密（发布签名私钥）。测试自己在文件头声明，不在这里
// 维护一份会过期的名单。这类测试**默认仍然跑**——维护者本机是有这些机密的，把它们
// 排除掉等于本地也失去覆盖；CI 用 --no-local-secrets 显式排除。
const LOCAL_SECRETS_PATTERN = /^\/\/\s*TEST-REQUIRES:\s*local-secrets\b/m;
// 诊断工具，不是通过/失败形式的测试。
...
function classify(fileName, source) {
  ...
  if (LOCAL_SECRETS_PATTERN.test(source)) return "local-secrets";
...
function parseArguments(argv) {
  const options = {
    ...
    browser: false,
    external: false,
    // 默认跑：维护者本机有这些机密。CI 上没有，也不该有，用 --no-local-secrets 排除。
    localSecrets: true,
    list: false,
    filter: null,
    timeoutMs: null,
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    } else if (argument === "--no-local-secrets") {
      options.localSecrets = false;
...
if (options.list) {
  ...
  for (const kind of ["unit", "browser", "external", "local-secrets", "helper", "diagnostic"]) {
...
const RUNNABLE_KINDS = ["unit", "browser", "external", "local-secrets"];
...
const selected = scripts.filter((script) => {
  ...
  // local-secrets 跟着 unit 走：它们本质是单元测试，只是多一个本机机密的前提。
  if (script.kind === "local-secrets" && (!options.unit || !options.localSecrets)) return false;
  if (!RUNNABLE_KINDS.includes(script.kind)) return false;
...
const skipped = scripts.filter(
  (script) => RUNNABLE_KINDS.includes(script.kind) && !selected.includes(script)
);
...
if (skipped.length > 0) {
  ...
  console.log(
    `Skipping ${skipped.length} (${skippedKinds}). Use --all / --external to include` +
      ` (local-secrets 需要维护者本机的发布签名私钥，被 --no-local-secrets 排除时无法补跑).`
  );
}
console.log("");
